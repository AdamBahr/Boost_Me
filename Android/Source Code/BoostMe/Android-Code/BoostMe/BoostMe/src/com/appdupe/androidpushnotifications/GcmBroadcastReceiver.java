...
public class GcmBroadcastReceiver extends WakefulBroadcastReceiver 
{
    ...
    public void onReceive(Context context, Intent intent) 
    {
      ...
      Log.i("GcmBroadcastReceiver...............", "push log onReceive push demo project...............  "+intent);
    ...
    private void fineLikedMatched(Bundle extras)
	{
		...
		logDebug("GcmBroadcastReceiver fineLikedMatched");
		...
		try {
			...
			{
					currentdeviceTime=mUltilitie.getCurrentGmtTime();
		...
		} catch (Exception e) 
		{
			...
			logError("GcmBroadcastReceiver fineLikedMatched Exception "+e);
		}
	...
	private class BackgroundTaskForFindLikeMatched extends AsyncTask<Bundle, Void, Void>
	{
		...
		protected Void doInBackground(Bundle... params) 
		{
			...
			File	 appDirectory = mUltilities.createAppDirectoy(con.getResources().getString(R.string.appdirectory));
			logDebug("GcmBroadcastReceiver   doInBackground appDirectory "+appDirectory);
			File	 _picDir  = new File(appDirectory, con.getResources().getString(R.string.imagedirematchuserdirectory));
			logDebug("GcmBroadcastReceiver doInBackground ");
			...
			////  likedmatchedata=   mUltilities.makeHttpRequest(CommanConstant.getliked_url,CommanConstant.methodeName,getuserparameter);
			logDebug("GcmBroadcastReceiver doInBackground   likedmatchedata "+likedmatchedata);
			//Gson gson = new Gson();
...
//	    "errMsg": "Matches found!",
		...
		//{
			...
			logDebug("GcmBroadcastReceiver doInBackground   likesList "+likesList);
			...
			likesList.add(likes);
			logDebug("GcmBroadcastReceiver doInBackground   imgaeName "+imgaeName);
			logDebug("GcmBroadcastReceiver doInBackground   imageUrl"+imageUrl);
			logDebug("GcmBroadcastReceiver doInBackground   userFName "+userFName);
			logDebug("GcmBroadcastReceiver doInBackground   senderFacebookId "+senderFacebookId);
			logDebug("GcmBroadcastReceiver doInBackground   activeFlag "+activeFlag);
			logDebug("GcmBroadcastReceiver doInBackground   userFaceBookidFromPush "+userFaceBookidFromPush);
			logDebug("GcmBroadcastReceiver doInBackground   curenttime "+curenttime);
			ArrayList<LikeMatcheddataForListview> arryList=new ArrayList<LikeMatcheddataForListview>();
				for (int i = 0; i < likesList.size(); i++) 
				{
					...
					File imageFile=	mUltilities.createFileInSideDirectory(_picDir, userName+facebookid+".jpg");
					 logDebug("GcmBroadcastReceiver doInBackground imageFile is profile "+imageFile.isFile());
					com.appdupe.flamer.utility.Utility.addBitmapToSdCardFromURL(likesList.get(i).getpPic().replaceAll(" ","%20"),imageFile);
					matcheddataForListview.setFilePath(imageFile.getAbsolutePath());
					arryList.add(matcheddataForListview);

				}
				logDebug("GcmBroadcastReceiver  listsize is arryList "+arryList.size());
				DatabaseHandler mDatabaseHandler =new DatabaseHandler(con);
				logDebug("GcmBroadcastReceiver  mDatabaseHandler "+mDatabaseHandler);
				SessionManager mSessionManager =new SessionManager(con);
				logDebug("GcmBroadcastReceiver  mSessionManager "+mSessionManager);
				String userFacebookid=mSessionManager.getFacebookId();
				logDebug("GcmBroadcastReceiver  userFacebookid "+userFacebookid);
				logDebug("GcmBroadcastReceiver  userFaceBookidfrompush "+userFaceBookidFromPush);
				
				boolean isdataiserted=	mDatabaseHandler.insertMatchList(arryList,userFacebookid);
				logDebug("GcmBroadcastReceiver  isdataiserted "+isdataiserted);
				
		 //  }
			}catch(Exception e)
			{
				e.printStackTrace();
				logError("GcmBroadcastReceiver  Exception "+e);
			}
